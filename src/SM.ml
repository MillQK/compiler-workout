...
| Expr.Call (f_name, args) -> List.concat (List.map compile_expr args) @ [CALL f_name]
...
| Stmt.Call (f_name, args) -> (
  ...
  compiled_args @ [CALL ("L" ^ f_name)]
)
| Stmt.Return m_v -> match m_v with Some v -> (compile_expr v) @ [END] | _ -> [END]

let rec compile_def (f_name, (f_args, f_locals, f_body)) = 
[LABEL ("L" ^ f_name); BEGIN (f_args, f_locals)] @ compile_stmt f_body @ [END]
